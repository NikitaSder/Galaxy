...
let item_descs  = ['Тарілка з принтом Марса. Доступна в червоному та білому кольорах', 'Кружка з принтом Марса. Доступна в червоному та білому кольорах', 'Масштабна модель однієї з ракет Galaxy', 'Масштабна модель марсохода (ми купуємо їх у NASA)']
let item_prices = ['1000', '1000', '2500', '5000']

for (let i = 0; i < item_name_objects.length; i += 1) {
    item_desc_objects[i].innerHTML  = item_descs[i]
    item_price_objects[i].innerHTML = item_prices[i]
}

let buttons = document.querySelectorAll('.add-to-cart-btn')
let cartCount = document.querySelector('.cart-amount')
let count = 0
